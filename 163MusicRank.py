...
def getTotalCommentById(musicId):
    ...
    try:
        req = requests.post(url, headers=headers, data=data, timeout=6)
...
if __name__ == "__main__":
    ...
    commetToalDict = {}
    for musicName in commentIdlist.keys():
        totalComment = getTotalCommentById(commentIdlist[musicName])
        commetToalDict[musicName.encode('utf-8')] = totalComment
    print commetToalDict
    commetToalDict = sorted(commetToalDict.items(), key=lambda e:e[1], reverse=True)
